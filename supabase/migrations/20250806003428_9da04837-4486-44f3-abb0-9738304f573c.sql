...
('Barbell Curl', 'Compound arm exercise', (SELECT id FROM exercise_categories WHERE name = 'Compound'), 'Arms', ARRAY[], 'strength', 'isolation', 'beginner', 'moderate', '["barbell"]'::jsonb, '["Stand with barbell", "Curl weight up", "Squeeze biceps", "Lower slowly"]'::jsonb, '["Keep elbows stable", "Full range of motion", "Control weight"]'::jsonb, '["Swinging weight", "Using momentum"]'::jsonb, false, false, false, 'Elbow flexion pattern', ARRAY['elbow pain'], true),

-- CORE EXERCISES
('Knees Down Plank', 'Modified plank for beginners', (SELECT id FROM exercise_categories WHERE name = 'Regressed'), 'Core', ARRAY[], 'strength', 'isometric', 'beginner', 'low', '[]'::jsonb, '["Kneel on ground", "Lower to forearms", "Keep straight line", "Hold position"]'::jsonb, '["Engage core", "Straight line", "Breathe normally"]'::jsonb, '["Sagging hips", "Holding breath"]'::jsonb, true, false, false, 'Core stabilization pattern', ARRAY['lower back pain'], true),
...
-- Update exercise performance analytics and history to include muscle group mappings
UPDATE exercises SET 
  muscle_group_primary = CASE
    WHEN name LIKE '%chest%' OR name LIKE '%pec%' OR name LIKE '%bench%' OR name LIKE '%push%' THEN 'Chest'
    WHEN name LIKE '%back%' OR name LIKE '%row%' OR name LIKE '%pull%' OR name LIKE '%lat%' THEN 'Back'
    WHEN name LIKE '%shoulder%' OR name LIKE '%press%' OR name LIKE '%raise%' THEN 'Shoulders'
    WHEN name LIKE '%curl%' OR name LIKE '%tricep%' OR name LIKE '%bicep%' THEN 'Arms'
    WHEN name LIKE '%plank%' OR name LIKE '%crunch%' OR name LIKE '%twist%' THEN 'Core'
    WHEN name LIKE '%squat%' OR name LIKE '%lunge%' OR name LIKE '%leg%' THEN 'Quadriceps'
    WHEN name LIKE '%deadlift%' OR name LIKE '%romanian%' THEN 'Hamstrings'
    WHEN name LIKE '%glute%' OR name LIKE '%hip%' THEN 'Glutes'
    WHEN name LIKE '%calf%' OR name LIKE '%heel%' THEN 'Calves'
    ELSE muscle_group_primary
  END
